<script>
  ...
  import type { Mutation } from "type-graphql";
  let parameters: XorParameters = {
    activationFunctions: [],
    generations: 10,
    mateChance: 0.4,
    mutations: [],
  ...
  async function startSimulation() {
    let response = await fetch("__REST_ENDPOINT__/start", {
      headers: {
  ...
  async function getParameters() {
    let response = await fetch("__REST_ENDPOINT__/configuration", {
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
      },
    ...
    const newParameter: XorParameters = await response.json();
  ...
  interface ActivationFunctionParameter {
    name: string;
    displayName: string;
    description: string;
    enabled: boolean;
  }
  interface XorParameters {
    generations: number;
  ...
  <div class="max-w-3xl mx-auto">
    <div class="bg-white overflow-hidden shadow rounded-lg">
      <div class="border-b border-gray-200 px-4 py-5 sm:px-6">
        Train a network {parameters.populationSize}
      </div>
